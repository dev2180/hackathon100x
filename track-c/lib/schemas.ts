...
export const IntakeSchema = z.object({
  ...
  experience: z.string().min(1, "Tell us about your building experience."),
  expertise: z.string().min(1, "Tell us your domain expertise."),
        ...
        nodes: {
          ...
          items: {
            ...
            properties: {
              ...
              gap:         { type: "string", description: "The specific thing THIS builder does not yet know at this node, calibrated to their stated experience and expertise. The knowledge this node bridges. For 'current' it can be the realization they're missing; for 'dead' it is the false belief that lures them in." },
              rabbitHole:  { type: "string", description: "How far down the rabbit hole to actually go here: a concrete time-box AND an explicit stop line, e.g. 'One afternoon — learn just enough to wire a single API call; do NOT study fine-tuning theory yet.' For 'dead' nodes, state plainly why going down this hole will not pay off for them right now." },
              depth:       { type: "string", enum: ["shallow", "moderate", "deep"], description: "The REAL effort this node deserves right now. Vary it across nodes to un-flatten the terrain — not every stage is equally hard. 'dead' nodes are usually 'deep' (a deep hole that is the wrong hole)." },
            },
            required: ["id", "label", "description", "type", "gap", "rabbitHole", "depth"],
            additionalProperties: false,
          },
        },
        edges: {
